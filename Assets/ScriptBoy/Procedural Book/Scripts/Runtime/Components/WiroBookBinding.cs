    ...
    public sealed class WiroBookBinding : BookBinding
    {
        ...
        class WiroBookBound : BookBound
        {
            ...
            public WiroBookBound(WireSetup wireSetup, Book book, Transform root, RendererFactory rendererFactory, MeshFactory meshFactory) : base(book, root)
            {
                ...
                Material wireMaterial = MaterialUtility.FixMetalMaterial(wireSetup.material);
            ...
            internal override PaperPattern CreatePaperPattern(int quality, Vector2 size, float thickness, PaperUVMargin uvMargin)
            {
                ...
                List<float> xList = new List<float>(nX);
                xList.Add(0);
                xList.Add(x1);
                xList.Add(x2);
                float xStep = (size.x - xList[2]) / (subdivideXSpace + 1);
                float xValue = xList[2] + xStep;
                ...
                List<float> zList = new List<float>(nZ);
                zList.Add(0);
                ...
                PaperMeshUtility.AddSeams(pattern, xList, zList, size, uvMargin);
                ...
                var leftSeam = pattern.leftSeam;
                if (leftSeam.active && leftSeam.index < 3)
                {
                    holeEndX++;
                    if (leftSeam.index < 2) holeStartX++;
                }

                int iH = 1 + subdivideZHole + subdivideZSpace0;

                borders.Add(new PaperBorder(0, 0, nX - 1, nZ - 1, false));

                var zMidSeam = pattern.zMidSeam;
                var downSeam = pattern.downSeam;
                var upSeam = pattern.upSeam;
                int holeIndex = 0;
                int holeStartZ = subdivideZSpace0 + 1;
                int holeEndZ = holeStartZ + subdivideZHole + 1;

                if (downSeam.active && downSeam.index <= holeEndZ)
                {
                    downSeam.active = false;
                    if (downSeam.index <= holeStartZ) holeStartZ++;
                    holeEndZ++;
                }
                ...
                for (int z = 0; z < nZ - 1; z++)
                {
                    for (int x = 0; x < nX - 1; x++)
                    {
                        ...
                        if (z >= holeStartZ && z < holeEndZ && x >= holeStartX && x < holeEndX) continue;
                    ...
                    if (z == holeEndZ)
                    {
                        ...
                        {
                            holeStartZ = z + subdivideZSpace + 1;
                            holeEndZ = holeStartZ + subdivideZHole + 1;

                            if (zMidSeam.active && holeStartZ >= zMidSeam.index)
                            {
                                holeStartZ++;
                                holeEndZ++;
                                zMidSeam.active = false;
                            }
                            ...
                            if (!downSeam.active && upSeam.active && upSeam.index <= holeEndZ)
                            {
                                if (upSeam.index <= holeStartZ) holeStartZ++;
                                holeEndZ++;
                                upSeam.active = false;
                            }
                        }
