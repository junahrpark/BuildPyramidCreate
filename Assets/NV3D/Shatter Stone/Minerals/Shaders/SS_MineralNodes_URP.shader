...
Shader "NV3D/SS_Minerals/URP/MineralNodes"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _Albedo_ST;
			float4 _RockTint;
			float4 _MineralTintBase;
			float4 _MineralTintNoise;
			float4 _Normal_ST;
			float4 _Emissive_ST;
			float4 _GlowTint;
						 ...
						 ) : SV_Target
			{
				...
				float2 texCoord52 = IN.ase_texcoord8.xy * float2( 1,1 ) + ( _DepthOffset * ase_tanViewDir ).xy;
				float2 panner12 = ( 1.0 * _Time.y * temp_cast_0 + texCoord52);
				...
				float3 BaseColor = ( ( ( fresnelNode23 * tex2DNode3.a ) * _FresnelStrength ) + lerpResult39 ).rgb;
				float3 Normal = UnpackNormalScale( tex2D( _Normal, uv_Normal ), 1.0f );
				float3 Emission = ( ( ( saturate( ( tex2DNode4 + tex2DNode9 ) ) * _GlowTint ) * tex2DNode1 * tex2DNode3.a ) * _GlowStrength ).rgb;
				float3 Specular = 0.5;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				ase_tanViewDir = normalize(ase_tanViewDir);
				float2 texCoord52 = IN.ase_texcoord5.xy * float2( 1,1 ) + ( _DepthOffset * ase_tanViewDir ).xy;
				float2 panner12 = ( 1.0 * _Time.y * temp_cast_0 + texCoord52);
				float4 tex2DNode9 = tex2D( _Depth_Noise, panner12 );
				float4 temp_output_29_0 = ( tex2DNode9 * 1.5 );
				float4 saferPower31 = abs( temp_output_29_0 );
				...
				float2 uv_Emissive = IN.ase_texcoord5.xy * _Emissive_ST.xy + _Emissive_ST.zw;
				float4 tex2DNode4 = tex2D( _Emissive, uv_Emissive );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float3 ase_tanViewDir =  tanToWorld0 * ase_worldViewDir.x + tanToWorld1 * ase_worldViewDir.y  + tanToWorld2 * ase_worldViewDir.z;
				ase_tanViewDir = normalize(ase_tanViewDir);
				float2 texCoord52 = IN.ase_texcoord3.xy * float2( 1,1 ) + ( _DepthOffset * ase_tanViewDir ).xy;
				float2 panner12 = ( 1.0 * _Time.y * temp_cast_0 + texCoord52);
				float4 tex2DNode9 = tex2D( _Depth_Noise, panner12 );
				float4 temp_output_29_0 = ( tex2DNode9 * 1.5 );
				float4 saferPower31 = abs( temp_output_29_0 );
...
WireConnection;61;3;3;3
WireConnection;61;4;3;2
WireConnection;31;0;29;0
ASEEND*/
//CHKSM=2DF8C01A76FCAC3A11A1BAFED41E289CFEB4597B